...
fn main() {
    ...
    let duration = env::args().nth(3).expect("missing duration for second");
    ffmpeg::init().unwrap();
    ...
    for (stream, mut packet) in ictx.packets() {
        if packet.is_key() && time::SystemTime::now().duration_since(begin ).unwrap() >time::Duration::new(duration.parse::<u64>().unwrap(), 0) {
            break;
        }
