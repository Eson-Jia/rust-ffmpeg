...
use ffmpeg::{codec, decoder, encoder, format, frame, log, media, picture, Dictionary, Packet, Rational, dict};
...
impl Transcoder {
    fn new(
        ...
        encoder.set_bit_rate(1024*1024/8);
...
fn main() {
    ...
    let mut octx = format::output_as(&output_file, "rtsp").unwrap();
    ...
    let mut first_frame_mapping: Vec<bool> = vec![true; ictx.nb_streams() as _];
    ...
    octx.write_header_with(dict!("rtsp_transport"=>"tcp")).unwrap();
